									 ...
									 ULONG *pulNonGatherMotions, BOOL *pfDML)
{
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		// but accesses out of bounds for INDF -- the crashing case for
		// `INTERSECT` on int8_tbl (NULL-safe semantics).  Delegate to
		// CPhysicalJoin::AlignJoinKeyOuterInner which handles both
		// shapes and also strips binary-coercible casts.
		...
		// type.  For equality, reuse pexprPred (correctly aligned by
		// AlignJoinKeyOuterInner above).  For INDF the DXL emitter
		// expects a ScalarBoolExpr(NOT, IsDistinctFrom(outer, inner)),
		// so reconstruct that with the aligned operands.
