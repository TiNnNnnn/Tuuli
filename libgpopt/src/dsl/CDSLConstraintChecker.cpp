...
//		Structural/property checks and ordered constraint dispatch. Legacy
//		subquery extraction lives in CDSLConstraintCheckerLegacySubquery.cpp.
//---------------------------------------------------------------------------
...
BOOL
CDSLConstraintChecker::FQueryDemandInsensitive(CExpression *pexpr)
{
	...
	const CDSLConstraint *pcon, const CDSLModel *pmodel) const
{
	...
	const BOOL fMatches = CDSLMatchView::FSameCapturedExpression(pexprResult, pexprExpected);
	...
	const CDSLConstraint *pcon, const CDSLModel *pmodel) const
{
	...
	const BOOL fMatches = CDSLMatchView::FSameCapturedExpression(pexprBound, pexprExpected);
	pexprExpected->Release();
	return fMatches;
}
	...
	for (ULONG ul = 0; nullptr != pdrgpwf && ul < pdrgpwf->Size(); ul++)
	{
		...
		if (CWindowFrame::IsEmpty(pwf) ||
			...
			(nullptr != pexprLeading &&
			 !CDSLMatchView::FSameCapturedExpression(pexprLeading, pwf->PexprLeading())) ||
			(nullptr != pexprTrailing &&
			 !CDSLMatchView::FSameCapturedExpression(pexprTrailing, pwf->PexprTrailing())))
		{
			return false;
		}
									 ...
									 const CDSLModel *pmodel) const
{
	...
	const BOOL fMatches = nullptr != pexprExpected &&
		CDSLMatchView::FSameCapturedExpression(pexprOut, pexprExpected);
									...
									const CDSLModel *pmodel) const
{
	...
	const BOOL fMergedValid = nullptr == pexprBoundMerged
		? EdslsideTarget == (*pdrgpsym)[0]->Eside()
		: CDSLMatchView::FSameCapturedExpression(pexprBoundMerged, pexprMerged);
	const BOOL fResidualValid = nullptr == pexprBoundResidual
		? EdslsideTarget == (*pdrgpsym)[1]->Eside()
		: CDSLMatchView::FSameCapturedExpression(pexprBoundResidual, pexprResidual);
	pexprMerged->Release();
								  ...
								  const CDSLModel *pmodel) const
{
	...
	switch (pcon->Edslcon())
	{
		...
		case EdslconFuncEq:
		{
			...
			{
				if (!CDSLMatchView::FSameCapturedExpression((*pdrgpexprFirst)[ul], (*pdrgpexprSecond)[ul]))
				{
					return false;
				}
			}
		...
		case EdslconWindowEq:
			return CDSLMatchView::FSameCapturedExpression(pmodel->PexprWindow(psymFirst),
