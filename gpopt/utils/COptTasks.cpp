...
extern "C" {
/* cdb/cdbvars.h not in PG18 */
/* optimizer/hints.h not in PG18 */
...
#define gp_session_id  0
#endif
#ifndef gp_command_count
#define gp_command_count  0
#endif
...
COptTasks::CreateOptimizerConfig(CMemoryPool *mp, ICostModel *cost_model)
{
	...
	ULLONG plan_id = 0; /* optimizer_plan_id is GPDB-only */
	...
	ULONG broadcast_threshold = (ULONG) 100000;  /* optimizer_penalize_broadcast_threshold */
	...
	ULONG dphyper_edge_budget = (ULONG) pg_orca_dphyper_edge_budget;
	ULONG dphyper_pair_budget = (ULONG) pg_orca_dphyper_pair_budget;
