...
#include "catalog/pg_operator_oids.h"
...
#include "parser/parse_func.h"
// #include "access/external.h"  -- Cloudberry external table, not in PG18
...
gpdb::AggregateExists(Oid oid)
{
	GP_WRAP_START;
	{
		/* PG18: check pg_proc directly */
		return SearchSysCacheExists1(PROCOID, ObjectIdGetDatum(oid)) &&
			   SearchSysCacheExists1(AGGFNOID, ObjectIdGetDatum(oid));
	}
	GP_WRAP_END;
	return false;
}
...
gpdb::MutateExpressionTree(Node *node, Node *(*mutator)(Node *, void *), void *context)
{
	GP_WRAP_START;
	{
		return expression_tree_mutator_impl(node, (tree_mutator_callback) mutator, context);
	}
	GP_WRAP_END;
	return nullptr;
}
...
gpdb::WalkExpressionTree(Node *node, bool (*walker)(Node *, void *), void *context)
{
	GP_WRAP_START;
	{
		return expression_tree_walker_impl(node, (tree_walker_callback) walker, context);
	}
	GP_WRAP_END;
	return false;
}
...
gpdb::GetCheckConstraintName(Oid check_constraint_oid)
{
	GP_WRAP_START;
	{
		...
		return get_constraint_name(check_constraint_oid);
	}
	GP_WRAP_END;
	return nullptr;
}
...
gpdb::GetCheckConstraintRelid(Oid check_constraint_oid)
{
	...
	{
		/* catalog tables: pg_constraint */
		/* PG18: look up conrelid from pg_constraint */
		HeapTuple	htup_cc = SearchSysCache1(CONSTROID, ObjectIdGetDatum(check_constraint_oid));
		if (!HeapTupleIsValid(htup_cc))
			elog(ERROR, "cache lookup failed for constraint %u", check_constraint_oid);
		Oid	relid_cc = ((Form_pg_constraint) GETSTRUCT(htup_cc))->conrelid;
		ReleaseSysCache(htup_cc);
		return relid_cc;
	}
	GP_WRAP_END;
	return 0;
...
gpdb::PnodeCheckConstraint(Oid check_constraint_oid)
{
	...
	{
		/* catalog tables: pg_constraint */
		/* PG18: look up conbin expression from pg_constraint */
		HeapTuple	htup_ex;
		Datum		val_ex;
		bool		isnull_ex;
		htup_ex = SearchSysCache1(CONSTROID, ObjectIdGetDatum(check_constraint_oid));
		if (!HeapTupleIsValid(htup_ex))
			elog(ERROR, "cache lookup failed for constraint %u", check_constraint_oid);
		val_ex = SysCacheGetAttr(CONSTROID, htup_ex, Anum_pg_constraint_conbin, &isnull_ex);
		Node *expr_ex = isnull_ex ? nullptr :
			(Node *) stringToNode(TextDatumGetCString(val_ex));
		ReleaseSysCache(htup_ex);
		return expr_ex;
	}
	GP_WRAP_END;
	return nullptr;
...
gpdb::GetCheckConstraintOids(Oid rel_oid)
{
	...
	{
		/* catalog tables: pg_constraint */
		/* PG18: scan pg_constraint for CHECK constraints on this relation */
		List	   *result_cc = NIL;
		SysScanDesc	scan_cc;
		HeapTuple	tuple_cc;
		Relation	conrel_cc = table_open(ConstraintRelationId, AccessShareLock);
		ScanKeyData skey_cc;
		ScanKeyInit(&skey_cc, Anum_pg_constraint_conrelid,
					BTEqualStrategyNumber, F_OIDEQ, ObjectIdGetDatum(rel_oid));
		scan_cc = systable_beginscan(conrel_cc, ConstraintRelidTypidNameIndexId,
									 true, NULL, 1, &skey_cc);
		while (HeapTupleIsValid(tuple_cc = systable_getnext(scan_cc)))
		{
			Form_pg_constraint con = (Form_pg_constraint) GETSTRUCT(tuple_cc);
			if (con->contype == CONSTRAINT_CHECK)
				result_cc = lappend_oid(result_cc, con->oid);
		}
		systable_endscan(scan_cc);
		table_close(conrel_cc, AccessShareLock);
		return result_cc;
	}
	GP_WRAP_END;
	return nullptr;
				  ...
				  Oid *cast_fn_oid, CoercionPathType *pathtype)
{
	GP_WRAP_START;
	{
		...
		/* PG18: use find_coercion_pathway */
		*pathtype = find_coercion_pathway(dest_oid, src_oid,
										  COERCION_IMPLICIT, cast_fn_oid);
...
gpdb::GetFuncArgTypes(Oid funcid)
{
	...
	{
		/* catalog tables: pg_proc */
		/* PG18: build arg types list from pg_proc.proargtypes */
		HeapTuple	ftup = SearchSysCache1(PROCOID, ObjectIdGetDatum(funcid));
		if (!HeapTupleIsValid(ftup))
			elog(ERROR, "cache lookup failed for function %u", funcid);
		Form_pg_proc proc = (Form_pg_proc) GETSTRUCT(ftup);
		List *argtypes = NIL;
		for (int i = 0; i < proc->pronargs; i++)
			argtypes = lappend_oid(argtypes, proc->proargtypes.values[i]);
...
gpdb::GetTypeName(Oid typid)
{
	...
	{
		/* catalog tables: pg_type */
		/* PG18: use format_type_be or look up typname directly */
		HeapTuple	ttup = SearchSysCache1(TYPEOID, ObjectIdGetDatum(typid));
		if (!HeapTupleIsValid(ttup))
			return nullptr;
		char *typname = pstrdup(NameStr(((Form_pg_type) GETSTRUCT(ttup))->typname));
...
gpdb::MakeSegmentFilterExpr(int segid)
{
	GP_WRAP_START;
	{
		/* PG18: no segment filter concept, return null */
		(void) segid; return nullptr;
	}
	GP_WRAP_END;
}
...
gpdb::NumericToDoubleNoOverflow(Numeric num)
{
	GP_WRAP_START;
	{
		/* PG18: use numeric_float8_no_overflow */
		PGFunction fn = numeric_float8_no_overflow;
		LOCAL_FCINFO(fcinfo_n, 1);
		InitFunctionCallInfoData(*fcinfo_n, nullptr, 1, InvalidOid, nullptr, nullptr);
		fcinfo_n->args[0].value = NumericGetDatum(num);
		fcinfo_n->args[0].isnull = false;
		Datum d = fn(fcinfo_n);
		return DatumGetFloat8(d);
	}
	GP_WRAP_END;
...
gpdb::IsOpNDVPreserving(Oid opno)
{
	switch (opno)
	{
		...
		/* case Int4DateAddOperator: same OID as DateInt4AddOperator, skip */
			return true;
		default:
			return false;
	}
								...
								int flags)
{
	GP_WRAP_START;
	{
		return query_or_expression_tree_walker_impl(node, (tree_walker_callback) walker, context, flags);
	}
	GP_WRAP_END;
	return false;
}
								  ...
								  int flags)
{
	GP_WRAP_START;
	{
		return query_or_expression_tree_mutator_impl(node, (tree_mutator_callback) mutator, context, flags);
	}
	GP_WRAP_END;
	return nullptr;
}
					  ...
					  int flags)
{
	GP_WRAP_START;
	{
		return query_tree_mutator_impl(query, (tree_mutator_callback) mutator, context, flags);
	}
	GP_WRAP_END;
	return nullptr;
}
					  ...
					  int flags)
{
	GP_WRAP_START;
	{
		return query_tree_walker_impl(query, (tree_walker_callback) walker, context, flags);
	}
	GP_WRAP_END;
	return false;
}
...
gpdb::IsChildPartDistributionMismatched(Relation rel)
{
	GP_WRAP_START;
	{
		/* child_distribution_mismatch is GPDB-only; return false in PG18 */
		(void) rel;
		return false;
	}
	GP_WRAP_END;
						...
						List *targetlist, Query *query, RangeTblEntry *rte)
{
	GP_WRAP_START;
	{
		/* BuildForeignScan is GPDB-only; use make_foreignscan in PG18 */
		(void) rel_oid;
		(void) query;
		(void) rte;
		return make_foreignscan(targetlist, qual, scanrelid,
								NIL, NIL, NIL, NIL, NULL);
	}
	GP_WRAP_END;
	return nullptr;
}
...
gpdb::CdbHashConstList(List *constants, int num_segments, Oid *hashfuncs)
{
	GP_WRAP_START;
	{
		/* cdbhash_const_list is GPDB MPP-only — single node always returns 0 */
		(void) constants; (void) num_segments; (void) hashfuncs;
		return 0;
	}
	GP_WRAP_END;
	return 0;
...
gpdb::CdbHashRandomSeg(int num_segments)
{
	GP_WRAP_START;
	{
		/* cdbhashrandomseg is GPDB MPP-only — single node always returns 0 */
		(void) num_segments;
		return 0;
	}
	GP_WRAP_END;
...
gpdb::HasUpdateTriggers(Oid relid)
{
	GP_WRAP_START;
	{
		/* has_update_triggers is GPDB-only; check via relation's trigdesc */
...
gpdb::GetOpFamiliesForScOp(Oid opno)
{
	GP_WRAP_START;
	{
		...
		/* get_operator_opfamilies is GPDB-only; query pg_amop directly */
		List *result = NIL;
		SysScanDesc scan;
		HeapTuple htup;
		Relation rel = table_open(AccessMethodOperatorRelationId, AccessShareLock);
		ScanKeyData skey;
		ScanKeyInit(&skey, Anum_pg_amop_amopopr, BTEqualStrategyNumber,
					F_OIDEQ, ObjectIdGetDatum(opno));
		scan = systable_beginscan(rel, AccessMethodOperatorIndexId, true, NULL, 1, &skey);
		while (HeapTupleIsValid(htup = systable_getnext(scan)))
		{
			Form_pg_amop amop = (Form_pg_amop) GETSTRUCT(htup);
			result = lappend_oid(result, amop->amopfamily);
		}
		systable_endscan(scan);
...
gpdb::GetCompatibleLegacyHashOpFamily(Oid opno)
{
	GP_WRAP_START;
	{
		/* get_compatible_legacy_hash_opfamily is GPDB-only */
		(void) opno;
		return InvalidOid;
	}
	GP_WRAP_END;
...
gpdb::GetRelAmName(Oid reloid)
{
	GP_WRAP_START;
	{
		/* GetAmName is GPDB-only; get the AM OID from pg_class then name */
		HeapTuple htup = SearchSysCache1(RELOID, ObjectIdGetDatum(reloid));
		if (!HeapTupleIsValid(htup))
			return nullptr;
		Form_pg_class relform = (Form_pg_class) GETSTRUCT(htup);
