...
```sh
python3 -m ml_orca.data.instantiate_query_workload \
...
```sh
python3 -m ml_orca.collect.run_workload_comparison --pg-config /path/to/pg_config \
...
```sh
python3 -m ml_orca.collect.profile_corpus_attempts --pg-config /path/to/pg_config \
...
```sh
python3 -m ml_orca.experiments.profile_rule_conditions \
...
python3 -m ml_orca.experiments.profile_rule_pair \
...
```sh
python3 -m ml_orca.experiments.profile_data_scale --pg-config /path/to/pg_config \
  --audit-bin build-ninja/pgorca_rule_audit --workload-dir output/selected-workload \
...
```sh
python3 -m ml_orca.experiments.compare_rule_curves \
...
```sh
python3 -m ml_orca.experiments.evaluate_rule_dimensions \
...
example translator through `../../../tools/ml-orca/experiments/ExportRuleExamples.java`. Supply
...
```sh
python3 -m ml_orca.experiments.export_rule_examples --output output/example-export \
...
```sh
python3 -m ml_orca.trace.profile_rule_candidates \
