...
from tools.learnable.collect.profile_corpus_attempts import summarize_trace, trace_run
...
from tools.learnable.common.paths import PGORCA_ROOT
...
def main():
    ...
    def recover_case(case):
        ...
        try:
            result = subprocess.run([sys.executable, '-u', '-B', '-m', 'tools.learnable.data.recover_history_corpus',
                '--corpus', str(root), '--case', case['case_id'], '--memory-gb', str(args.memory_gb)]
                + (['--defer-failed-traces'] if args.defer_failed_traces else []),
                timeout=args.timeout, capture_output=True, text=True, cwd=PGORCA_ROOT)
