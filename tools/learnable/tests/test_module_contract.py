...
from tools.learnable.__main__ import COMMANDS
from tools.learnable.common.paths import LEARNABLE_ROOT, PGORCA_ROOT, TEST_ASSETS, package_sources, source_file
from tools.learnable.objectives.policy import objective_channels, policy_target
...
class ModuleContractTest(unittest.TestCase):
            ...
            for node in ast.walk(ast.parse(path.read_text())):
                ...
                for name in names:
                    with self.subTest(file=str(path), dependency=name):
                        self.assertFalse(name == 'test' or name.startswith(('test.', 'tools.learnable.tests.')))
                        if path.parent.name in ('models', 'objectives'):
                            self.assertFalse(name.startswith(('tools.learnable.training', 'tools.learnable.collect')))
                        if path.parent.name == 'models':
                            self.assertFalse(name.startswith('tools.learnable.objectives'))
                        if path.parent.name == 'objectives':
                            self.assertFalse(name.startswith(('torch', 'tools.learnable.models')))
    ...
    def test_commands_and_asset_paths(self):
        for command, module in COMMANDS.items():
            ...
            with self.subTest(command=command):
                self.assertTrue(callable(importlib.import_module('tools.learnable.' + module).main))
        from tools.learnable.collect.run_workload_comparison import DEFAULT_WORKLOADS, DEFAULT_POLICY
        self.assertTrue(DEFAULT_WORKLOADS.is_dir())
        self.assertTrue(DEFAULT_POLICY.is_file())
        self.assertTrue((TEST_ASSETS / 'rules/orca_replacements.rules').is_file())
        self.assertTrue((LEARNABLE_ROOT / 'collect/trace_corpus.sh').is_file())
        for path in package_sources().values():
        ...
        self.assertEqual(LEARNABLE_ROOT, PGORCA_ROOT / 'tools/learnable')
    ...
    def test_cli_help_from_outside_repository_without_site_packages(self):
        ...
        result = subprocess.run([sys.executable, '-B', '-S', str(LEARNABLE_ROOT), '--help'],
