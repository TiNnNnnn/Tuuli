...
from tools.learnable.encoding.group_expression_encoding import group_expression_tree
from tools.learnable.trace.profile_rule_candidates import candidate_state
from tools.learnable.encoding.rule_history_encoding import attach_history
from tools.learnable.encoding.rule_policy_encoding import encode_sequence, fit_vocabulary
from tools.learnable.tests.test_group_expression_encoding import attempt_fixture
from tools.learnable.tests.test_rule_policy_encoding import ir_fixture
from tools.learnable.tests.test_rule_tree_model import tree_features, torch
...
def history_fixture():
    with patch('tools.learnable.tests.test_rule_tree_model.encode_sequence', side_effect=lambda s, _: s):
...
def attach(feature, bundle, timestamp='2026-09-12T11:00:00+00:00', allowed=None):
    ...
    with patch('tools.learnable.encoding.rule_history_encoding.read_snapshot', side_effect=lambda s: json.dumps(documents[s['path']]).encode()), \
         patch('tools.learnable.encoding.rule_history_encoding.input_sequences', return_value=deepcopy(feature)):
...
class RuleHistoryEncodingTest(unittest.TestCase):
    def test_training_minimum_counts_complete_dynamic_queries_and_rejects_leakage(self):
        from tools.learnable.training.train_policy_baseline import history_training_gate
    ...
    def test_shared_history_equals_independent_admission_and_keeps_time_gate(self):
        from tools.learnable.training.train_policy_baseline import reuse_history
...
class RuleHistoryModelTest(unittest.TestCase):
    def test_edge_gather_preserves_full_model_gradients_and_adam_updates(self):
        from tools.learnable.models.rule_tree_model import TreePolicyPredictor
    ...
    def test_context_moments_distinguish_spread_and_preserve_counts_and_gradients(self):
        from tools.learnable.models.rule_tree_model import context_summary
