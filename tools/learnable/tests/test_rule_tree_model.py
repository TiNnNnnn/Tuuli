...
from tools.learnable.encoding.rule_policy_encoding import input_sequences, rule_structure, rule_root_index, encode_sequence, fit_vocabulary
from tools.learnable.tests.test_rule_policy_encoding import ir_fixture, catalog_fixture
...
def tree_features(irs, paths=('r',), vocabulary=None):
    ...
    with patch('tools.learnable.encoding.rule_policy_encoding.read_snapshot', side_effect=lambda s: json.dumps(documents[s['path']]).encode()):
...
class RuleTreeModelTest(unittest.TestCase):
    ...
    def test_recursive_tree_topology_order_and_arbitrary_child_count(self):
        from tools.learnable.models.rule_tree_model import RuleTreeEncoder
