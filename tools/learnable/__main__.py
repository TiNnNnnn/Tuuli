...
def main():
    if len(sys.argv) < 2 or sys.argv[1] in ('-h', '--help'):
        print('usage: python3 -m tools.learnable COMMAND [arguments]\n\nCommands:')
        for name, module in COMMANDS.items():
            print(f'  {name:18} {module}')
        return 0
    command = sys.argv.pop(1)
    if command not in COMMANDS:
        raise SystemExit('unknown learnable command: ' + command)
    return importlib.import_module('tools.learnable.' + COMMANDS[command]).main()
