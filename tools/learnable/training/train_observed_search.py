...
from tools.learnable.objectives.observed_search import TARGETS, observed_target
...
def main():
    ...
    if args.resume:
        ...
        for item in items:
            if (item['case'] != cases[item['case']['case_id']]
                    ...
                    or observed_target(json.loads(read_snapshot(item['summary_snapshot']))) != item['target_log1p_ms']):
                raise ValueError('resume query, split or timing label changed')
    ...
    model = ObservedSearchPredictor(len(vocabulary), args.width, 'static', 3, history=True,
                                    history_mode=args.mode)
    optimizer = torch.optim.Adam(model.parameters(), lr=.003)
    train = [i for i in items if i['split'] == 'train']
    validation = [i for i in items if i['split'] == 'validation']
    constant = torch.tensor([sum(i['target_log1p_ms'][j] for i in train) / len(train) for j in range(2)])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for item in order[offset:]:
            ...
            optimizer.zero_grad()
            loss = torch.nn.functional.smooth_l1_loss(model(data), torch.tensor(item['target_log1p_ms']))
        ...
        model.eval()
        errors, baseline, val_loss = torch.zeros(2), torch.zeros(2), 0.
        with torch.inference_mode():
            for item in validation:
                log_progress({'phase': 'validation', 'epoch': epoch + 1, 'steps': steps,
                              'case_id': item['case']['case_id']})
                expected = torch.tensor(item['target_log1p_ms'])
