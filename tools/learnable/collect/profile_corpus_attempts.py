...
from tools.learnable.common.paths import TEST_ASSETS, source_file
from tools.learnable.experiments.plot_stats_sweep import chinese_plotting
from tools.learnable.trace.profile_rule_candidates import (binding_origin_evidence, candidate_evidence, post_search_evidence,
                                     state_coverage, STAGES)
from tools.learnable.collect.run_dphyper_stability import imported_cases
from tools.learnable.collect.run_trace_corpus import render_trace_query, orca_fallback_reason
from tools.learnable.common.artifacts import artifact_snapshot
from tools.learnable.collect.run_workload_comparison import dsl_observability, inventory, trace_records
...
def partition_history(items, seed):
    ...
    from tools.learnable.experiments.profile_query_cohort import query_features
...
def summarize_trace(text, rc, fallback, required_binding_version=None, require_stats=False, run=None):
    ...
    timeline = None
    if require_stats:
        from tools.learnable.encoding.group_expression_encoding import stats_timeline
        ...
        if path.exists():
            ...
            try:
                ...
                if args.history_split and result['complete']:
                    from tools.learnable.encoding.rule_history_encoding import encode_observations
